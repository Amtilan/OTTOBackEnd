...
class RecommendationGenerator:
    ...
    def _search_product_by_title(self, title: str) -> Optional[dict]:
        for product in self.product_catalog:  # Берем список продуктов
...
⚠️ ВАЖНО:  
- Все 20 продуктов должны быть выбраны ИСКЛЮЧИТЕЛЬНО из каталога, который предоставлен ниже.  
- НЕ ДОБАВЛЯЙ продукты, которых нет в каталоге.  
- ВЫБИРАЙ товары, которые соответствуют указанным проблемам кожи.  
- Если точной информации о проблемах нет, выбирай продукты, подходящие для всех типов кожи.  

📌 Отчет о состоянии кожи:  
{self.analysis_report}  

📌 Каталог продуктов (выбирай только отсюда):  
{json.dumps(self.product_catalog, ensure_ascii=False, indent=4)}  

Рекомендации:  
- Проанализируй состояние кожи (например, акне, темные круги, морщины) и порекомендуй ровно 20 продуктов, которые решают эти проблемы, учитывая тип кожи пользователя.  
- Если информация недоступна, отдавай предпочтение продуктам, подходящим для всех типов кожи.  
- Убедись, что рекомендации разнообразны и включают различные типы продуктов.  
- В список ОБЯЗАТЕЛЬНО включи:  
  - Минимум 2 очищающих средства  
  - Минимум 2 продукта из категории «другое»  
  - Минимум 2 сыворотки  
  - Минимум 1 тонер  
- Каждая рекомендация должна включать название, цену и изображение.  
"""
